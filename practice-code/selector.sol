...
contract SelectorFun {
    // 调用智能合约的本质是 给目标合约发送一段calldata,发送的calldata中的前4个字节是函数选择权(selector)
    ...
    // 入参：to：0x2c44b726ADF1963cA47Af88B284C06f30380fC78
    function mint(address to) external {
        emit Log(msg.data);
    }
    // calldata的作用：告诉智能合约，在调用哪个函数，参数是什么。method id、selector和函数签名
    // 输出selector：address
    function mintSelector() external pure returns(bytes4 mSelector){
        return bytes4(keccak256("mint(address)"));
        // 函数签名中，uint和int要写为uint256、int256
    }
